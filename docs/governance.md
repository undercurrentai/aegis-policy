...
- **SDK error_class taxonomy source-of-truth**: `aegis-governance/aegis-sdk/src/aegis/_verify_local.py` (Apache-2.0, distributed via PyPI as `aegis-governance>=0.6.1[verify]`). The `error-class-parity.yml` workflow auto-checks parity against the latest published wheel.
