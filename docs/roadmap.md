...
| **Sprint 6 / F1** | ☐ planned | `aegis-governance/.github/workflows/aegis-deploy.yml` integration (dogfood) via the Sprint 5/E3 reusable workflow (job-level `workflow_call` invocation). aegis-governance becomes its own first verifier-kit consumer. CLAUDE.md §8 Ask-First gate. | Separate plan + PR (on `aegis-governance`) |
