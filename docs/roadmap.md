...
| **v1.3.0 (test-suite gating + changelog reconciliation)** | ✅ shipped 2026-07-29 | Closes the two findings deferred from the 2026-07-28 /quality-gate run. **(1) No CI job ran `tests/`** — the only pytest invocation in the repo was `e2-action-selftest.yml` (single file, `workflow_dispatch`-only), so 90+ tests including the API-key redaction guard were laptop-only. New `.github/workflows/tests.yml` gates the suite on 3.12 + 3.13, with secrets-needing tests deselected by a `needs_secrets` marker (`pytest.ini`) rather than a path `--ignore`, and a step that asserts the guard file **executed** ≥20 tests by reading the JUnit XML the suite step produced. The first cut of that step counted `--collect-only` output; `security-auditor` and `reliability-auditor` independently proved it bypassable and both bypasses were reproduced (a module-level `pytest.mark.skip` → 24 skipped, exit 0, green, collected=24). **(2) CHANGELOG drift 9 PRs deep** — one consolidated `[1.3.0]` entry rather than nine retroactive versions. `docs-auditor` corrected the bump level: PATCH → **MINOR**, because #32 added two optional inputs to the public `aegis-enforce.yml` surface, which is the MINOR row of `docs/release-discipline.md` verbatim. Also fixed in-flight: the sanitizer could not match the masked key shape its own docstring cited (`sk-pr***…***dEfA` — `*` outside the character class), and the reviewer's **success** path published model output to a public PR comment with no sanitization at all. 98 tests green, 19 deselected. | aegis-policy v1.3.0 |
