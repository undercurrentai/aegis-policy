...
class TestCodexOutputPathRegression:
    """The Codex lane wrote its review where nobody looked.

    `openai/codex-action` forwards `output-file` verbatim to
    `codex exec --output-last-message`, and the CLI resolves that path against
    `--cd` — which this workflow sets to the BASE checkout subdirectory. A bare
    `codex_review.md` therefore landed at `aegis-policy-base/codex_review.md`,
    while all eight downstream consumers read `./codex_review.md` at the runner
    root.

    Result: "Codex review file missing or empty — fail closed" on EVERY PR that
    ran the lane (#30, #32, #33, #34). The review was always produced; it was
    never where the workflow looked. Four consecutive red checks read as a
    broken reviewer rather than a broken path, which is why it survived so long.

    The gpt-5.4-pro lane already had this right (`--output ../gpt_review.md`);
    only the codex lane was wrong, so a working sibling masked the bug.
    """

    def _codex_step(self) -> dict:
        jobs = yaml.safe_load(AI_SECOND_REVIEW_WORKFLOW.read_text())["jobs"]
        step = next(
            (
                s
                for s in jobs["codex-review"]["steps"]
                if isinstance(s.get("uses"), str) and "codex-action" in s["uses"]
            ),
            None,
        )
        assert step is not None, "codex-review job has no codex-action step"
        return step

    def test_output_file_escapes_the_working_directory(self):
        """`output-file` must climb out of `--cd` to reach the runner root."""
        with_block = self._codex_step()["with"]
        output_file = str(with_block["output-file"])
        working_dir = str(with_block.get("working-directory", "")).strip()

        if working_dir and working_dir not in (".", "./"):
            assert output_file.startswith("../"), (
                f"output-file {output_file!r} resolves against --cd "
                f"({working_dir!r}), so it lands in that subdirectory — but the "
                "Enforce verdict / Load review body / Post comment steps all read "
                "it at the runner root. Prefix with '../'."
            )

    def test_consumers_agree_on_the_runner_root_path(self):
        """Every consumer reads the bare filename; the producer must match."""
        text = AI_SECOND_REVIEW_WORKFLOW.read_text()
        # The producer is the only reference allowed to carry a '../' prefix.
        assert "../codex_review.md" in text, "producer no longer writes to runner root"
        assert text.count("../codex_review.md") == 1, (
            "exactly one reference (the producer) should be '../'-prefixed; "
            "consumers run at the runner root and must use the bare filename"
        )

    def test_prompt_file_stays_workspace_relative(self):
        """The asymmetry is intentional, not a typo to 'fix' later.

        `prompt-file` is read by the action in-process (cwd = $GITHUB_WORKSPACE),
        so it keeps its subdirectory prefix; `output-file` is resolved by the CLI
        against `--cd`. Someone normalizing these two to match would re-break the
        lane.
        """
        with_block = self._codex_step()["with"]
        prompt_file = str(with_block["prompt-file"])
        assert not prompt_file.startswith("../"), (
            "prompt-file is workspace-relative and must NOT be '../'-prefixed"
        )
        assert prompt_file.startswith(str(with_block["working-directory"]).strip()), (
            "prompt-file should be prefixed with the BASE checkout directory"
        )
