...
**The reduced-quorum release.** **MINOR** per the bump table: a new opt-in capability (repo-variable-switched single-lane review) with a backward-compatible default plus new gate-step outputs (`quorum_mode`, `quorum_overdue`) surfacing in the audit trail. With the variable unset, the DECISION behavior is today's tri-AI quorum unchanged; the posted approval body gains a machine-readable `<!-- quorum:full -->` marker (an additive audit line, not a semantic change — the audit comment is byte-identical in full mode). Plus bows the v1.4.x/v1.5.0 proof runs surfaced. No consumer-facing contract change; check-name strings untouched.

### Added

- **Reduced-quorum mode (`AEGIS_REVIEW_QUORUM=claude-only`) — ADR-004.** Both OpenAI reviewer lanes have been dead on every run since the platform account went unfunded, so §54.5 rule 1 (ABSENT → BLOCK) correctly fail-closed every routine PR into a manual §34.17.2 break-glass cycle — the fail-closed design working as intended, at a standing operational tax with no risk-reduction return. A new repo variable now switches the aggregator to judging routine PRs on the Claude lane alone, under a TIGHTENED envelope: verdict must be `APPROVE` (a COMMENT that passes under full quorum blocks solo), Max Concern must be NONE/LOW (a MEDIUM that passes under full quorum blocks solo), and the diff must be ≤ 25 files / ≤ 800 changed lines (measured from the same paginated `files` array the reviewed-surface rule uses). ONLY the exact literal `claude-only` activates — unset/empty/`full`/typo yield today's behavior on BOTH sides (job-skip inequality; aggregator strict-equality with `full` fallback), and a mode/lane consistency check BLOCKS any run where the variable flipped mid-flight (both TOCTOU directions land fail-closed). The Claude lane is never skippable in any mode. Trust-spine carve-out, reviewed-surface ⊆ rule, self-tune D-lock, TOCTOU head-SHA re-check, and PAT gating are quorum-independent, positions test-pinned. Every decision reason, audit comment, and approval body carries a `[REDUCED QUORUM: claude-only]` banner + machine marker + the restore command (`gh variable set AEGIS_REVIEW_QUORUM --body full`); past the ADR's review-by date (2026-09-28) the branch emits a warning and an OVERDUE banner line — warn, never block. What is LOST — three-model error decorrelation collapses to one model inside the tightened envelope — is stated plainly in `docs/architecture/adr/ADR-004-reduced-quorum-claude-only-2026-07-30.md` with the exit condition (OpenAI funded + both lanes green on a probe PR). Enforced by `TestReducedQuorumMode`, including the re-parenthesized fork-guard shape on the OpenAI jobs' `if:` (GH Actions `&&` binds tighter than `||` — without the parens the quorum clause would silently rewire the fork guard).
- **Aggregator rule 1.5: truncated-diff gate (G4 wire-through)** — found by this release's own pre-commit audit (HIGH, adversarially confirmed). Each reviewer lane's in-job G4 enforce step already fails on truncated-diff-APPROVE, but the lanes are `continue-on-error` and the verdict output is extracted BEFORE enforcement — so the aggregator received `verdict=APPROVE, result=success` for a diff the model provably did not fully see, in EVERY quorum mode since Phase 2 shipped. The 25/800 single-lane caps don't cover it (one minified file passes both caps at >200 KB). Each lane now exports `diff_truncated` as a job output and the aggregator BLOCKs when any ACTIVE lane saw a truncated diff — quorum-independent, placed between rule 1 and the carve-out, test-pinned with its own negative controls. Also hardened from the same audit: the mode computation reads the variable raw (no trim — GHA's `!=` doesn't trim either), with the one known parser asymmetry (GHA case-insensitive `!=` vs JS case-sensitive `===`) documented in place: every divergent value lands BLOCK, never approve.
- **`e3-all-asserts-green` summary job** (`e3-workflow-selftest.yml`) — a fully-passing e3 run still concludes `failure` at the RUN level by design (negative-pair verify jobs exit 1; reusable-workflow jobs can't `continue-on-error` at step level), so the run badge can never say "e3 passed". The new job can: it requires all four assert signals (`assert-happy-path`, `assert-tampered-digest`, `assert-expired`, and `selftest-replay-second` — the replay pair's 4th signal is the selftest job itself, which asserts in-job) to conclude `success`, under `if: ${{ !cancelled() }}` so a red dependency yields a red summary, never a skipped one.
- **Vendored-SDK drift guard.** `scripts/_verify_local_vendored.py` gains a machine-readable `# Proven-against: aegis-governance==1.4.0 (2026-07-30)` header line; a new invariant test asserts it equals the exact pin in `requirements-verify.txt`. Every deliberate SDK bump now mechanically forces the re-verification ritual (re-diff the vendored copy against the released source, update the line) — the failing test is the enforcement, closing §26.17 FU-3 in spirit.
- **Dependabot pip hardening.** The pip ecosystem entry gains `ignore: aegis-governance` (deliberate-bump-only — an auto-filed bump PR could never merge: it can't update the Proven-against header, can't re-prove the 19 fixtures, and trips the drift guard; it would only sit red) with the accepted cost stated in the config comment: `ignore` also suppresses security-update PRs for that one dependency, compensated by alerts remaining visible, the `cryptography>=48.0.1` floor, and the alert-#1 house precedent. The rest of the pip surface groups into one weekly `python-ci-tooling` PR.

### Fixed

- **Consumer `@path`-envelope examples in the reusable-workflow docs could never have worked** — the follow-up the v1.5.0 e3 fix recorded in `docs/roadmap.md`. `REUSABLE-WORKFLOW.md` (TL;DR + both worked examples + the `envelope` input description) and `README.md`'s reusable-workflow section showed `envelope: "@artifacts/envelope.json"`, but the reusable workflow's verify job runs in its own fresh workspace (an aegis-policy checkout) where consumer-repo paths never resolve — proven by e3's first-ever dispatch failing file-not-found on exactly this pattern. Examples now pass the envelope INLINE from a prior job's `$GITHUB_OUTPUT`, with a warning box explaining why. **Surgical, not blanket** (the e3 lesson — the three input kinds resolve against different bases): the composite-Action examples keep their `@`-paths, which are CORRECT there (the composite runs inside the consumer's own job), now annotated with why the two surfaces differ.
...
- **`aegis-sdk-version` default `1.0.0` → `1.4.0`** in BOTH public surfaces — the composite (`actions/verify-aegis-attestation/action.yml`) and the reusable workflow (`.github/workflows/aegis-verify-attestation.yml`) — plus every doc that displayed the old default. Anything taking the default now installs `aegis-governance[verify]==1.4.0`, resolving `cryptography` **48.0.1** (out of the GHSA-537c-gmf6-5ccf range) instead of 46.0.7. Blast radius at merge: near-zero, verified — zero external composite consumers found by code search (hedged: private third-party repos are invisible to search), the one external reusable-workflow consumer (aegis-governance's deploy `verify` job) is SHA-pinned AND passes `aegis-sdk-version: "1.0.0"` explicitly, and the `@v1` camp is empty; the bump changes what consumers get when they next move their pin. New `TestSdkDefaultParity` invariant pins the two declaration sites EQUAL, closing the two-site-strand hazard the roadmap warned about (no test pinned either default before).

### Fixed

- **`e3-workflow-selftest.yml` could never have passed — its envelope paths predate the self-checkout pattern.** Proven by its FIRST-EVER dispatch (2026-07-30, at the 1.0.0 default, deliberately baselined BEFORE the bump so failures could not be misattributed): all four reusable-workflow verify invocations failed `AttestationEnvelopeShapeInvalid: [Errno 2]`. The reusable workflow self-checks-out aegis-policy into the `aegis-policy/` SUBDIR of a fresh workspace (§37.18 resolver), while e3 passed workspace-root-relative `@tests/fixtures/...` envelope paths written before that pattern existed. **The fix is surgical, not uniform — the first draft blanket-prefixed all 13 paths and the pre-commit gate caught it (HIGH + 2 MEDIUM, fully static-traced): the three input kinds resolve against DIFFERENT bases in `verify_action.py`** — envelope `@`-paths against `GITHUB_WORKSPACE`, override paths against `REPO_ROOT` (the checked-out tree). So exactly the 4 reusable-invocation envelope paths gain the `aegis-policy/` prefix; the 8 override paths stay repo-root-relative; the composite-direct replay-second job (root checkout) stays fully unprefixed. The baseline's own failure signature proves the split: `AttestationEnvelopeShapeInvalid` is only reachable AFTER keys/policy load, so the overrides were already resolving. The reusable workflow's override-input descriptions said "workspace-relative" — the wording that induced the broken first fix — and are corrected. e3 green is claimed nowhere in this entry; the post-merge dispatch is the proof. (e2's same-day baseline: all 4 jobs green on first dispatch ever.) Consumer-facing corollary in `docs/roadmap.md`: a consumer `@path` envelope can never resolve in the reusable workflow's fresh workspace — inline JSON is the working pattern; the docs' `@artifacts/...` examples need a follow-up.

## [1.4.1] — 2026-07-30

**Dependabot alert #1 closed end-to-end.** **PATCH** per the bump table: CI-lockfile + docs scope, no consumer-facing contract change (inputs/outputs/check-names untouched). The v1.4.0 notes recorded the alert as "blocked upstream"; the block turned out to be an **undispatched release**, not a missing fix: aegis-governance main had carried the widened `[verify]` cryptography cap (`>=48.0.1,<49`, #271 — the same GHSA-537c-gmf6-5ccf, fixed server-side in June) and an unreleased SDK 1.4.0 since 2026-06-12. On 2026-07-30 that release's notes were reconciled (aegis-governance#300) and `sdk-publish.yml` dispatched; PyPI 1.4.0 is live.

### Fixed

- **Quality-gate review of the v1.4.0 ship + this bump: 40 findings (2 HIGH, 15 MEDIUM, 23 LOW), every one remediated in place, zero deferred.** Four independent reviewers (three partitioned bug-hunt sweeps + a docs auditor) over the full v1.4.0 delta and this diff. The two HIGHs: **(1)** the auto-approve aggregator never required changed-files ⊆ reviewed-surface, so a PR bundling a reviewed file with a file outside both `GATED_PATHS` and the carve-out (e.g. a new root `conftest.py` — auto-executed by pytest in every CI venv) could be machine-approved sight-unseen — now a hard BLOCK (`REVIEWED_SURFACE` subset rule), with `conftest.py` pre-armed in both defense layers and two invariants incl. a GATED_PATHS↔JS mirror check; **(2)** the §37.17 cross-repo-checkout regression guard was re-breakable — comments and a `core.info` string satisfied its substring positives while the actual env wiring mutated back to `github.workflow_sha` (the historical bug, at the historical site) — now pinned to the wiring shape with a `github.*` negative. Among the MEDIUMs: the Bearer-token sanitizer missed the base64 `+/=` alphabet (token tails were publishable — empirically demonstrated); the shadow-eval lane posted to the internal-only raw Cloud Run URL with every failure path silently green (now custom domain + loud DEGRADED annotations); the advisory verdict check-run attached to the invisible merge-commit SHA (now PR head); and seven mutation-survivable test guards (file-global install invariant, line-wrapping-blind write guard, condition-only rename check, tautological `splitlines` assertion, cwd-coupled workspace test, single-occurrence JUnit pins, no-fail-pin codeowners guard) were each hardened and re-proven — **13 negative controls fired** (V1–V7 + the six prior-round re-runs), files restored byte-identical. All three `resolve_callee` copies gained full regex escaping + a refuse-on-unpinnable-ref throw, byte-identical parity maintained.

### Changed

- **`requirements-verify.txt` / `requirements-verify-ci.txt`: `aegis-governance[verify]` 1.0.0 → 1.4.0**, resolving `cryptography` 46.0.7 → **48.0.1** (out of the GHSA-537c-gmf6-5ccf vulnerable range). A permanent `cryptography>=48.0.1` floor is added to the source file so a future SDK range loosening can never silently resolve back into the vulnerable window. The bump followed the exact-pin policy to the letter: wire format documented byte-identical since 1.0.0; `_verify_local` drift since the vendored parity ref proven formatting-only; the 19 fixture tests re-verified against the locally-built 1.4.0 wheel **before** the PyPI release was dispatched, and again against the published PyPI artifact (hermetic wheel-only install, `pip check` clean) before this merge. CI-lockfile scope only — **`action.yml`'s consumer-facing default (`aegis-sdk-version: 1.0.0`) is deliberately unchanged**; bumping it is a consumer-contract decision tracked in `docs/roadmap.md`.

## [1.4.0] — 2026-07-29

> **Known open vulnerability, recorded at release time.** Dependabot alert #1 (HIGH) fired minutes after `requirements-verify-ci.txt` first landed: it pins `cryptography==46.0.7`, inside the vulnerable-bundled-OpenSSL range (< 48.0.1). A patched floor is **unsatisfiable today** — every released `aegis-governance[verify]` (1.0.0–1.3.1) caps `cryptography<47`. The alert is deliberately left open rather than dismissed; the unblock is an upstream aegis-governance release with a wider cap, tracked in `docs/roadmap.md` with the exposure assessment (offline fixture verification on an ephemeral no-secret runner; the vulnerable surface is TLS code the tests never exercise).

**The close-the-gaps release.** Three PRs (#37, #39, #40) landing the four items v1.3.0 left open, one day later — plus everything their audits surfaced (33 confirmed findings across three adversarially-verified auditor passes, every one remediated before its commit). The through-line: **signals that did not mean what a reader assumed.** A required check that never reports looked pending (it blocked forever); a green suite looked like coverage (19 tests ran nowhere); a lockfile-less `pip install` looked reproducible (it resolved live); `result=success` looked like a working reviewer lane (its infrastructure was dead); a solo CODEOWNERS line looked permanent (its replacement's validity now lives in org settings — so it got a guard). **MINOR** per the bump table: new workflow job (`verifier-kit`), new job outputs (`step_outcome`), new guard capabilities — all backward-compatible; no consumer-facing contract change (`aegis-enforce.yml` inputs/outputs and the resolved check-name are untouched). Out-of-band with this release, org ruleset `16294975` went 5 → 7 required contexts, and the org team `@undercurrentai/security-reviewers` (id `18755567`) was created and seeded. Verified end-to-end with a throwaway probe PR (#38, closed unmerged): all 7 required contexts reported on a docs-only PR — the first in-band-satisfiable PR shape in this repo's history — and a deliberate failing test blocked the merge.
